...
MatchedDataMap <- function(x, plot.title = '') {
  # Function that plots a map of the US of the matched power plants with
  # color corresponding to positive/negative difference of outcome in matched
  # pairs and lines connecting matched pairs.
  #
  # Args:
  #  x: The pairs matrix of all matching functions.
  
  require(ggplot2)
  
  x <- as.data.frame(x)
  
  point_data <- data.frame(lon = c(x$Trt.Fac.Longitude, x$Con.Fac.Longitude),
                           lat = c(x$Trt.Fac.Latitude, x$Con.Fac.Latitude))
  point_data$col <- 1
